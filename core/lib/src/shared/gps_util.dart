...
import 'interface/scheduler_listener.dart';
...
class GpsUtil {
  ...
  static Future<void> onStream({interval = 10000}) async {
    ...
    if (hasPermission) {
      FlLocation.getLocationStream(
              interval: interval, accuracy: LocationAccuracy.high)
          .handleError((error) {})
          .listen((event) {
  ...
  static void force(
      ...
      required GpsListener gpsListener,
      Function()? onIsMock}) async {
    await Scheduler()
  ...
  static double calculateDistance(
    ...
    var c = cos;
    var a = 0.5 -
        c((latitudeCompare - currentLatitude) * p) / 2 +
        c(currentLatitude * p) *
            c(latitudeCompare * p) *
            (1 - c((longitudeCompare - currentLongitude) * p)) /
            2;
